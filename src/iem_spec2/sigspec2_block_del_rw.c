...
typedef struct _sigspec2_block_delwrite
{
	...
	t_symbol	*x_sym;
	int				x_max_blocks;
	int				x_w_schedule_counter;
	int				x_r_schedule_counter;
	int				x_first;
	int				x_write_index;
	t_float		x_msi;
...
static void sigspec2_block_delwrite_dsp(t_sigspec2_block_delwrite *x, t_signal **sp)
{
	...
	x->x_first = 1;
	post("delwrite");
	x->x_r_schedule_counter = 0;
	x->x_w_schedule_counter = 0;
	if(n&15)
...
static void sigspec2_block_delread_dsp(t_sigspec2_block_delread *x, t_signal **sp)
{
	...
	if(delwriter)
	{
		if(n == delwriter->x_blocksize)
		{
			...
			if(x->x_read_index < 0)
				x->x_read_index += x->x_max_blocks;
			delwriter->x_r_schedule_counter = 0;
