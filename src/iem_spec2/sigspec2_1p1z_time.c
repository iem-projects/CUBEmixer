...
/* -------------------------- spec2_1p1z_time~ ------------------------------ */
static t_class *sigspec2_1p1z_time_class;

typedef struct _sigspec2_1p1z_time
{
	t_object	x_obj;
	t_float		x_a0;
	t_float		x_a1;
	...
	int				x_blocksize;
	t_float		x_msi;
} t_sigspec2_1p1z_time;

static void sigspec2_1p1z_time_list(t_sigspec2_1p1z_time *x, t_symbol *s, int argc, t_atom *argv)
{
	if((argc >= 3) &&
		IS_A_FLOAT(argv,0) &&
		IS_A_FLOAT(argv,1) &&
...
static t_int *sigspec2_1p1z_time_perform(t_int *w)
{
	t_float *in = (float *)(w[1]);
	t_float *out = (float *)(w[2]);
	t_sigspec2_1p1z_time *x = (t_sigspec2_1p1z_time *)(w[3]);
	int i, n = (t_int)(w[4]);
	t_float a0 = x->x_a0;
	t_float a1 = x->x_a1;
	t_float b1 = x->x_b1;
...
static t_int *sigspec2_1p1z_time_perf16(t_int *w)
{
	t_float *in = (float *)(w[1]);
	t_float *out = (float *)(w[2]);
	t_sigspec2_1p1z_time *x = (t_sigspec2_1p1z_time *)(w[3]);
...
static void sigspec2_1p1z_time_dsp(t_sigspec2_1p1z_time *x, t_signal **sp)
{
	int n = (sp[0]->s_n)/2;
	...
	if(n&15)
		dsp_add(sigspec2_1p1z_time_perform, 4, sp[0]->s_vec, sp[1]->s_vec, x, n);
	else
		dsp_add(sigspec2_1p1z_time_perf16, 4, sp[0]->s_vec, sp[1]->s_vec, x, n);
}

static void *sigspec2_1p1z_time_new(t_symbol *s, int argc, t_atom *argv)
{
	t_sigspec2_1p1z_time *x = (t_sigspec2_1p1z_time *)pd_new(sigspec2_1p1z_time_class);

	outlet_new(&x->x_obj, &s_signal);
	x->x_blocksize = 0;
	x->x_begmem_forw = (t_float *)0;
	if(argc >= 3)
		sigspec2_1p1z_time_list(x, s, argc, argv);
	else
	{
		x->x_a0 = 1.0f;
		x->x_a1 = 0.0f;
...
static void sigspec2_1p1z_time_free(t_sigspec2_1p1z_time *x)
{
	if(x->x_begmem_forw)
...
{
	sigspec2_1p1z_time_class = class_new(gensym("spec2_1p1z_time~"), (t_newmethod)sigspec2_1p1z_time_new, (t_method)sigspec2_1p1z_time_free,
		sizeof(t_sigspec2_1p1z_time), 0, A_GIMME, 0);
	CLASS_MAINSIGNALIN(sigspec2_1p1z_time_class, t_sigspec2_1p1z_time, x_msi);
	class_addlist(sigspec2_1p1z_time_class, (t_method)sigspec2_1p1z_time_list);
	class_addmethod(sigspec2_1p1z_time_class, (t_method)sigspec2_1p1z_time_dsp, gensym("dsp"), 0);
	class_sethelpsymbol(sigspec2_1p1z_time_class, gensym("iemhelp2/help-spec2_1p1z_time~"));
}
