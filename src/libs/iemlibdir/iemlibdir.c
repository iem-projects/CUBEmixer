...
static int iemlibdir_doit(t_canvas *canvas, char *fullclassname, char*classname)
{
    int fd = -1;
    ...
    if(canvas) 
    {
      t_canvasenvironment *canvasenvironment = canvas_getenv(canvas);
        ...
        canvasenvironment->ce_path = namelist_append(canvasenvironment->ce_path, 
                                                     pathbuf, 0);
        post("iemlibdir_loader: added '%s' to the canvas-local objectclass path", classname);
    }
    else
#endif
    {
        ...
        post("iemlibdir_loader: added '%s' to the global objectclass path", classname);
//        post("\tThis is deprecated behavior.");
    }
    /* post("iemlibdir_loader loaded fullclassname: '%s'\n", fullclassname); */
    if (sys_verbose) 
        post("Loaded iemlibdir '%s' from '%s'", classname, dirbuf);
...
static int iemlibdir_loader(t_canvas *canvas, char *classname)
{
    unsigned int i;
...
/* look for meta file (classname)/(classname)-meta.pd */
    ...
    if (!iemlibdir_doit(canvas, fullclassname, classname)) {
        strncpy(fullclassname, classname, FILENAME_MAX -6);
        strcat(fullclassname, "-meta");

        return (iemlibdir_doit(canvas, fullclassname, classname));
    }
    
    return (1);
}

void iemlibdir_setup(void)
{
/* relies on t.grill's loader functionality, fully added in 0.40 */
#if (PD_MINOR_VERSION >= 40)
    sys_register_loader(iemlibdir_loader);
#else
    error("ERROR: to function, iemlibdir needs to be compiled against Pd 0.40 or higher,\n");
    post("\tor a version that has sys_register_loader()");
#endif
    post("iemlibdir loader %s",version);  
    post("\twritten by Hans-Christoph Steiner <hans@at.or.at>");
    post("\tmldified by IOhannes m zmoelnig <zmoelnig@iem.at>");
