...
static void create_widget(t_popup *x, t_glist *glist)
{
  ...
  //  DEBUG("id: .x%x.c.s%x", canvas, x);
  DEBUG("create_widget end");
}
...
static void popup_drawme(t_popup *x, t_glist *glist, int firsttime)
{
  ...
  //  DEBUG("drawme %d",firsttime);
     if (firsttime) {
       //       DEBUG("glist %x canvas %x",x->x_glist,canvas);
